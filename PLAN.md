...
| D21 | `tweepy` (API-key) auth path for X / Twitter | §4.8 | §4.8.x or §4.11 (`[adapters.twitter]`) | A user with API keys / a config flag to select backend | Spec says "optional `tweepy`". §4.8 ships twikit-only — adding tweepy now means a second client class, a second auth flow, and a runtime backend selector. The handler shape (timeline/like, timeline/reply, dms/*) is the same; tweepy would slot in as an alternate `TwitterClientProtocol` impl. Most natural trigger is a `[adapters.twitter].auth = "tweepy" \| "twikit"` knob in §4.11, with env-var fallback (e.g. `TWITTER_BEARER_TOKEN`). Until then, twikit covers the cookie path the spec called the default. |
| D22 | twikit method-name verification (`get_home_timeline`, `favorite_tweet`, `create_tweet(reply_to=…)`, `get_dm_threads`, `get_dm_messages`) | §4.8 | §4.8.x or §4.13 | context7 access / real-account smoke | Direct parallel to D20: §4.8 used twikit method names from convention without context7 cross-check (no quota at implementation time). Handler tests pass with hand-rolled fakes. First manual smoke against a real X account is the verification — and a one-shot patch if names diverged. The DM handlers also exercise the try-and-degrade path, so wrong DM method names will fall into the "DMs unavailable" branch silently — worth noting in the §4.13 smoke list. |
| D23 | X DMs availability cached at session level (today: probed per-call) | §4.8 | §4.8.x | Performance complaint or UX awkwardness | Today every `dms/threads` and `dms/messages` call tries the twikit endpoint and degrades on failure. If a user's account never has DMs, this is one wasted RPC per `d` press — fine for v1. Caching `dmsAvailable` for the lifetime of the sidecar process would skip the probe but adds invalidation complexity (re-probe after a Premium upgrade?). Not worth it until someone asks. |

**Pickup ordering when §4.11 lands.** D4 → D5 → D6/D10 → D12 → D18 are mechanical wiring (D6 and D10 are the same row from different sections; D18 ties IG creds to config). D1 is a small design choice (copy + threshold). D9 is only relevant if D1 picks the persistent-count variant. D2/D11 unblock together when §4.7 or §4.9 wires its first rich-render adapter; D15/D16 are the §4.7-specific shapes of those. D8 unblocks independently. **D13 is now resolved** by §4.7 (`extras: ["instagram"]` on three descriptors). D14 unblocks when §4.12 picks up CI. D17 only matters if users complain about per-tab spawn cost. D19 is UX polish on top of D13. D20 / D22 unblock on next context7 access or first real-account smoke (parallel rows for IG and X respectively). D21 lands with §4.11's `[adapters.twitter]` block. D23 only matters if users complain about per-call probe cost. **§4.5 D7 is also resolved** — `scroll-*` routing landed in §4.6 T11.
